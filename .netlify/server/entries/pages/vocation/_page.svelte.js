...
const SkillCard = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$result.css.add(css);
  return `<section class="skill-cards svelte-1s4gj47"><div class="section-wrapper svelte-1s4gj47">${validate_component(Chip, "Chip").$$render(
    $$result,
    {
      chipText: "our skills",
      chipIcon: "verified",
    ...
    {},
    {}
  )} <div class="cards-wrapper svelte-1s4gj47">${each(skillCards, (skill) => {
    return `<article class="skill-card svelte-1s4gj47"><span class="material-symbols-sharp svelte-1s4gj47">${escape(skill.icon)}</span> <h4 class="title svelte-1s4gj47">${escape(skill.title)}</h4> </article>`;
  })}</div></div> </section>`;
});
