...
const ContentBlock = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$result.css.add(css);
  return `<section class="content-render svelte-1xdoccz"><div class="${[
    "content-wrapper svelte-1xdoccz",
    (blockLayout ? "blockLayout" : "") + " " + (flexReverse ? "flexReverse" : "")
  ].join(" ").trim()}"><div class="block-content svelte-1xdoccz">${validate_component(Chip, "Chip").$$render(
    $$result,
    {
    ...
    {},
    {}
  )} ${heroBlock ? `<h1 class="content-title svelte-1xdoccz">${each(highlightText(contentTitle, contentTitleHighlight), (word) => {
    return `<span class="${["svelte-1xdoccz", word.isHighlighted ? "highlight" : ""].join(" ").trim()}">${escape(word.text)} </span>`;
  })}</h1>` : `<h2 class="content-title svelte-1xdoccz">${each(highlightText(contentTitle, contentTitleHighlight), (word) => {
    return `<span class="${["svelte-1xdoccz", word.isHighlighted ? "highlight" : ""].join(" ").trim()}">${escape(word.text)} </span>`;
  })}</h2>`} <p class="content-des svelte-1xdoccz">${escape(contentDescription)}</p> ${ctaButtons ? `<div class="content-cta-container svelte-1xdoccz">${each(ctaButtons, (cta) => {
  ...
  })}</div>` : ``}</div> <div class="content-media svelte-1xdoccz">${parseContentMediaType(contentMedia).type === "video" ? `<video muted playsinline disablepictureinpicture controls><source${add_attribute("src", contentMedia, 0)} type="video/mp4"></video>` : `<img${add_attribute("src", contentMedia, 0)} alt="" class="svelte-1xdoccz">`}</div></div> </section>`;
});
export {
  ContentBlock as C
};
